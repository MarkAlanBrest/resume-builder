...
export async function getStorageDiagnostics() {
  ...
  if (hasBlobStorage()) {
    try {
      const courseOfStudy = await readBlobJson("course-of-study.json");
      blobReadOk = true;
      blobHasData = courseOfStudy !== null;
    } catch {
      blobReadOk = false;
      blobHasData = false;
    }
...
export async function readJsonFile(filename, fallback) {
  ...
  if (hasPostgresStorage()) {
    let db = await readPostgresJson(filename);
    if (db === null && bundled !== null) {
      await writePostgresJson(filename, bundled);
      return bundled;
    }
  ...
  if (hasBlobStorage()) {
    const blob = await readBlobJson(filename);
    if (blob !== null) return blob;
    return bundled ?? fallback;
  }

  if (hasGithubStorage()) {
    const github = await readGithubJson(filename);
...
export async function writeJsonFile(filename, data) {
  if (hasPostgresStorage()) {
    return writePostgresJson(filename, data);
  }
