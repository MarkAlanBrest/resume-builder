...
function applyLegacyProgramMigration(store) {
  const migrated = { ...store };

  for (const [programKey, entry] of Object.entries(store)) {
    if (!entry || !isLegacyProgramKey(programKey)) continue;

    const campusKeys = expandLegacyProgramKey(programKey);
    const hasCampusSpecificData = campusKeys.some((key) => migrated[key]);

    if (hasCampusSpecificData) continue;

    for (const campusKey of campusKeys) {
      if (!migrated[campusKey]) migrated[campusKey] = entry;
    }
  }
...
function removeSupersededLegacyKeys(store) {
  ...
  for (const programKey of Object.keys(store)) {
    ...
    const campusKeys = expandLegacyProgramKey(programKey);
    if (campusKeys.some((key) => cleaned[key])) {
      delete cleaned[programKey];
    }
  }
...
async function purgeSupersededLegacyKeys(storeId, store) {
  for (const programKey of Object.keys(store)) {
    ...
    const campusKeys = expandLegacyProgramKey(programKey);
    if (campusKeys.some((key) => store[key])) {
...
function buildStore(storeId, runtime, bundled) {
  ...
  return applyLegacyProgramMigration(store);
}
