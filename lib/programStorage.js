...
async function deletePostgresProgram(storeId, program) {
  if (!hasPostgresStorage()) return;
  await ensurePostgresTable();
  const sql = getSql();
  await sql`
    DELETE FROM admin_program_data
    WHERE store_id = ${storeId} AND program = ${program}
  `;
}
...
function splitLegacyKeys(store) {
  const result = { ...store };

  for (const [programKey, entry] of Object.entries(store)) {
    if (!entry || !isLegacyProgramKey(programKey)) continue;

    for (const campusKey of expandLegacyProgramKey(programKey)) {
      if (!result[campusKey]) {
        result[campusKey] = { ...entry };
      }
    }
    delete result[programKey];
  }

  return result;
}

function removeSupersededLegacyKeys(store) {
  return splitLegacyKeys(store);
}

async function purgeSupersededLegacyKeys(storeId, store) {
  for (const programKey of Object.keys(store)) {
    if (!isLegacyProgramKey(programKey)) continue;
    await deletePostgresProgram(storeId, programKey);
  }
}
...
async function migrateRuntimeLegacyKeys(storeId, runtime) {
  if (!runtime || !Object.keys(runtime).some(isLegacyProgramKey)) {
    return runtime;
  }

  const splitRuntime = splitLegacyKeys(runtime);
  await writeRuntimeStore(storeId, splitRuntime);
  await purgeSupersededLegacyKeys(storeId, splitRuntime);
  return splitRuntime;
}

async function readStore(storeId) {
  const bundled = readBundledStore(storeId);
  const runtime = await migrateRuntimeLegacyKeys(
    storeId,
    await readRuntimeStore(storeId)
  );
...
export async function readProgramEntry(storeId, program) {
  ...
  if (store[program]) return store[program];
  return readBundledProgram(storeId, program) ?? null;
}

export async function writeProgramEntry(storeId, program, data) {
  const store = removeSupersededLegacyKeys(await readStore(storeId));
  store[program] = data;
  const cleanedStore = removeSupersededLegacyKeys(store);
  await writeRuntimeStore(storeId, cleanedStore);
  await purgeSupersededLegacyKeys(storeId, cleanedStore);
