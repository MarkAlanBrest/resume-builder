...
function buildOverlayHeaderXml(name, nameColor, pageColor) {
...
<w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office">
  <w:p><w:pict>
      ...
      </v:shape></w:pict></w:p></w:hdr>`;
}

const EMPTY_FIRST_HEADER_XML = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"/>';
...
async function patchCornerOverlayBlob(buffer, overlay) {
  ...
  rels = rels.replace("</Relationships>", `<Relationship Id="${defaultRelId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/header" Target="header1.xml"/></Relationships>`);
  const firstRelId = nextOverlayRelId(rels);
  rels = rels.replace("</Relationships>", `<Relationship Id="${firstRelId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/header" Target="header2.xml"/></Relationships>`);
  zip.file(relsPath, rels);
  zip.file("word/header1.xml", buildOverlayHeaderXml(overlay.name, overlay.nameColor, overlay.pageColor));
  zip.file("word/header2.xml", EMPTY_FIRST_HEADER_XML);
  let ct = await zip.file("[Content_Types].xml").async("string");
  ct = ct.replace(/<Override[^>]*PartName="\/word\/(header|footer)\d+\.xml"[^>]*\/>/g, "");
  if (!ct.includes("/word/header1.xml")) ct = ct.replace("</Types>", '<Override PartName="/word/header1.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.header+xml"/></Types>');
  if (!ct.includes("/word/header2.xml")) ct = ct.replace("</Types>", '<Override PartName="/word/header2.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.header+xml"/></Types>');
  ...
  docXml = docXml.replace(/<w:sectPr[\s\S]*?<\/w:sectPr>/, (sect) => {
    ...
    const refs = `<w:headerReference w:type="first" r:id="${firstRelId}"/><w:headerReference w:type="default" r:id="${defaultRelId}"/>`;
    ...
    s = s.replace(/<w:pgMar\b[^>]*\/>/g, (m) => {
      let out = m.replace(/\bw:footer="[^"]*"/, 'w:footer="0"');
...
async function main() {
  ...
  console.log("header1 colors:", (await zip.file("word/header1.xml").async("string")).includes('w:val="FFFFFF"'));
  console.log("no footers:", !zip.file("word/footer1.xml"));
  console.log("margin:", (await zip.file("word/header1.xml").async("string")).includes("margin-right:0.65in"));
}
